...
import { X, Truck, Package, MapPin, CheckCircle, Loader2 } from "lucide-react";
...
export default function TrackingModal({ isOpen, onClose, waybill, invoice }: TrackingModalProps) {
  ...
  const fetchTracking = async () => {
    setLoading(true);
    try {
        const res = await fetch(`/api/shipping/track/dummy`);
    //   const res = await fetch(`/api/shipping/track/${waybill}`);
      const data = await res.json();
      // Sesuaikan mapping data.history sesuai response JSON asli Lion Parcel
      setTrackingData(data?.result || data); 
    } catch (err) {
      console.error("Error fetching Lion Parcel:", err);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white w-full max-w-lg rounded-none shadow-sm overflow-hidden border border-gray-100 flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 bg-gray-50/50 flex justify-between items-center border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-700 font-normal tracking-wide">Lion Parcel</p>
          </div>
          <div className="text-right">
            <p className="text-[9px] uppercase tracking-widest text-gray-400 mb-1">No. Resi (STT)</p>
            <p className="text-xs text-[#FF5722] font-normal tracking-[0.1em]">{waybill}</p>
          </div>
        </div>
        ...
        <div className="flex-1 p-8 overflow-y-auto no-scrollbar min-h-[300px]">
          {loading ? (
            ...
            <div className="relative space-y-10">
              {/* Mapping History dari API Lion Parcel */}
              {trackingData?.history?.map((step: any, idx: number) => (
                <div key={idx} className="relative flex gap-8">
                  {/* Line Vertikal */}
                  {idx !== trackingData.history.length - 1 && (
                    <div className="absolute left-[9px] top-6 w-[1px] h-[calc(100%+24px)] bg-gray-50" />
                  )}
                  
                  {/* Circle Marker - Norvine Sharp Style */}
                  <div className={`relative z-10 w-[19px] h-[19px] flex items-center justify-center border bg-white ${idx === 0 ? 'border-[#FF5722]' : 'border-gray-100'}`}>
                    <div className={`w-[5px] h-[5px] ${idx === 0 ? 'bg-[#FF5722]' : 'bg-gray-100'}`} />
                  </div>
                  ...
                  <div className="flex-1 -mt-1">
                    ...
                    <p className="text-[11px] text-gray-500 font-normal leading-relaxed">
                      {step.location || "Lokasi tidak terdeteksi"}
                    </p>
                  </div>
                </div>
              ))}
