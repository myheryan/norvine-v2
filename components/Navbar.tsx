...
import { FiInstagram, FiMenu, FiShoppingCart } from 'react-icons/fi'
import { Five, Four, NorvineLogo, One, Three, Two } from './icons'
import { NavbarContext } from './context/NavbarContext'
import { Desktop, Mobile } from './responsive'
import { useCart } from './context/CartContext'
import UserMenu from '@/components/UserMenu' // Import Komponen Baru
      ...
      <Mobile>
        <nav className={`fixed top-0 left-0 right-0 z-50 flex h-16 items-center justify-between px-5 transition-all ${useSolidBg ? 'bg-[#1D1E20] text-white shadow-md' : 'bg-transparent text-slate-900'}`}>
          ...
          <div className="flex items-center">
            <Link href="/cart" className="relative"><FiShoppingCart size={22} />
            {cartCount > 0 && <span className="absolute -top-3 -right-2 flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-[9px] font-bold text-white ring-1 ring-white">{cartCount}</span>}
          </Link>
            <UserMenu textColor={getTextColor()} />
          </div>
        </nav>
        {/* Drawer Mobile Logic Tetap Sama */}
        <div className={`fixed inset-0 z-[60] bg-white px-8 py-8 transition-all duration-500 ${!openMobileNav ? 'opacity-0 pointer-events-none -translate-y-full' : 'opacity-100 translate-y-0'}`}>
