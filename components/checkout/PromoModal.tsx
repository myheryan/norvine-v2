      ...
      >
        ...
        <div className="px-6 py-5 border-b border-zinc-100 flex items-center justify-between bg-white">
          <div>
            ...
            <p className="text-xs text-zinc-400 mt-0.5">Pilih promo untuk pesananmu</p>
          </div>
          <button 
            type="button"
            onClick={onClose} 
        ...
        <div className="flex-1 overflow-y-auto bg-zinc-50/30 p-6 space-y-8 scrollbar-hide">
          ...
          <section className="space-y-3">
            <label className="text-[11px] font-bold text-zinc-500">Punya kode promo?</label>
            <div className={`flex bg-white border transition-all ${hasActivePromo ? 'border-zinc-900' : 'border-zinc-200 focus-within:border-zinc-400'}`}>
              <input 
                ...
                className="flex-1 px-4 py-3 bg-transparent outline-none text-sm placeholder:text-zinc-300 disabled:text-zinc-900 font-bold"
              />
              {hasActivePromo ? (
                <button 
                  type="button"
                  onClick={handleRemovePromo} 
                  className="px-6 py-3 text-red-600 text-xs font-black hover:bg-red-50 transition-colors"
                >
                  Lepas
                </button>
              ) : (
                <button 
                  ...
                  className="px-6 py-3 bg-zinc-900 text-white text-xs font-black disabled:bg-zinc-100 disabled:text-zinc-400 transition-all active:scale-95"
                >
                  {isApplying ? '...' : 'Pakai'}
                </button>
              )}
          ...
          <section className="space-y-4">
            <h4 className="text-[11px] font-bold text-zinc-500">Promo Tersedia</h4>
            
            {isFetchingList ? (
              <div className="py-20 flex flex-col items-center justify-center gap-3">
                <FiLoader className="animate-spin text-zinc-900" size={24} />
                <span className="text-xs font-bold text-zinc-400">Memuat...</span>
              </div>
            ) : (
              <div className="space-y-3">
                {availablePromos.length === 0 ? (
                    <div className="py-10 text-center border border-dashed border-zinc-200 bg-white">
                        <p className="text-xs text-zinc-400 font-bold">Belum ada promo tersedia</p>
                    </div>
                  ...
                  return (
                    ...
                    >
                      <div className={`w-20 flex flex-col items-center justify-center border-r border-dashed border-zinc-200 ${isSelected ? 'bg-zinc-900 text-white' : 'bg-zinc-50 text-zinc-300'}`}>
                        <FiTag size={20} />
                        <span className="text-[8px] font-black mt-1">NORVINE</span>
                      </div>
                      ...
                      <div className="flex-1 p-4 flex flex-col justify-between">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <span className={`text-xs font-black transition-colors ${isSelected ? 'text-red-600' : 'text-zinc-900'}`}>
        ...
        <div className="p-6 bg-zinc-50 border-t border-zinc-100">
          <div className="flex items-start gap-3">
            ...
            <p className="text-xs font-bold text-zinc-400 leading-loose">
